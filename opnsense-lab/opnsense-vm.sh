...
create_template() {
    ...
    # Bare VM shell — VGA std; access via Proxmox noVNC console
    ...
    # Stub NICs; net0=LAN (vtnet0), net1=WAN (vtnet1) — matches OPNsense default mapping
    qm set $TEMPLATE_ID --net0 virtio,bridge=$LAN_BRIDGE
    qm set $TEMPLATE_ID --net1 virtio,bridge=$WAN_BRIDGE
...
create_vm() {
    ...
    # OPNsense assigns LAN to vtnet0 (net0) and WAN to vtnet1 (net1) by default.
    # Wire bridges to match: net0→vmbr1 (LAN), net1→vmbr0 (WAN)
    qm set $VMID --net0 virtio,bridge=$LAN_BRIDGE   # vtnet0 = LAN  10.0.33.1
    qm set $VMID --net1 virtio,bridge=$WAN_BRIDGE   # vtnet1 = WAN  192.168.28.182

    # Config ISO injection disabled — configure manually via console menu + web UI
...
case "${1:-help}" in
    ...
    create)
        ...
        sleep 30
        wait_for_web "$WAN_IP" || true  # probe WAN; PVE host is not on vmbr1/LAN subnet

        echo ""
        echo "ℹ️  Access (from vmbr1 / k0s network):"
        echo "    Web UI:  https://${LAN_IP}     root / ${ROOT_PASSWORD}"
        echo "    Console: https://$(hostname -I | awk '{print $1}'):8006 → VM ${VMID} → Console"
        echo ""
